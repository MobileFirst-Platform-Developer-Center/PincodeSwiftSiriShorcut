...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([UIUserActivityRestoring]?) -> Void) -> Bool {
        if let intent = userActivity.interaction?.intent as? GetBalanceIntent {
            handle(intent)
            return true
        }
        return false
    ...
    private func handle(_ intent: GetBalanceIntent) {
         let handler = GetBalanceIntentHandler()
...
//
//        completion(GetBalanceIntentResponse.success(user: user!, balance: "One Crore"))
//    }
